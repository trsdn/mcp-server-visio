...
- **Use for ALL COM Interop work** - pages, shapes, stencils, ShapeSheet cells, connectors, windows, and document properties
...
- Particularly valuable for: Shapes, pages, masters/stencils, ShapeSheet cells, connectors, and complex COM scenarios
...
public async Task<OperationResult> ComplexAsync(IVisioBatch batch, string name)
{
    ...
    {
        return await batch.Execute((ctx, ct) => {
            temp = ((dynamic)ctx.Document).Pages.Item(1).DrawRectangle(1.0, 1.0, 3.0, 2.0);
            // ... operation ...
            return ValueTask.FromResult(new OperationResult { Success = true });
        });
    }
...
**All document close and Visio quit operations use `VisioShutdownService` with resilient retry:**
...
**Shutdown Order:**
1. **Optional Save** - If `save=true`, calls `document.Save()` explicitly before close
2. **Close Document** - Calls `document.Close()` (save param controls Visio's prompt behavior)
3. **Release Document** - Releases COM reference via `ComUtilities.Release()`
...
- **Outer timeout (30s)**: Overall cancellation for Visio.Quit() - catches hung Visio (modal dialogs, deadlocks)
...
- Handles edge cases: disconnected COM proxies, hung Visio, modal dialogs
...
```
- **30s quit timeout**: Catches truly hung Visio (modal dialogs, deadlocks) via CancellationToken
...
try
{
    ...
    ComUtilities.Release(ref shape!);  // ❌ Won't execute if exception above!
...
int pageCount = document.Pages.Count;  // Runtime error risk: COM numeric value may be double
int shapeCount = page.Shapes.Count;    // Runtime error risk: COM numeric value may be double

// ✅ CORRECT: Explicit conversion required
int pageCount = Convert.ToInt32(document.Pages.Count);
int shapeCount = Convert.ToInt32(page.Shapes.Count);
```

**Common Properties Affected:**
- `Pages.Count` / `Shapes.Count` → COM numeric values; convert explicitly
...
for (int i = 1; i <= Convert.ToInt32(shapes.Count); i++) { var shape = shapes.Item(i); }
```
...
shape.Text = "Hello";
shape.CellsU("FillForegnd").FormulaU = "RGB(255,255,0)";
```
...
| Not releasing objects | `try/finally` + `ComUtilities.Release()` |
