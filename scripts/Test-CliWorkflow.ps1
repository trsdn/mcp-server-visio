...
# This smoke test drives real Visio COM. Without an installed Visio there is
# nothing to exercise, so skip instead of blocking the commit. The ProgID is
# registered by every Visio SKU that exposes the automation model.
if (-not [Type]::GetTypeFromProgID('Visio.Application')) {
    Write-Host "`nSKIP: Microsoft Visio is not installed on this machine." -ForegroundColor Yellow
    Write-Host "      The CLI workflow smoke test requires a real Visio installation." -ForegroundColor Yellow
    exit 0
}
...
Test-Step "Create page" {
    & $cli -q page create --session $sessionId --position 0 --name "Smoke Test" | ConvertFrom-Json
} -Verify {
    param($r)
    $r.success -eq $true
}
...
#    This step would catch deployment issues like missing office.dll (a missing Office interop dependency) because
#    VisioBatch.ctor runs AutomationSecurity setup before opening any document.
...
if ($reopenSession -and $reopenSession.sessionId) {
    ...
    $reopenedShapes = Test-Step "List shapes in reopened page 1" {
