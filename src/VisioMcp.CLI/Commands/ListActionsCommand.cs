...
internal sealed class ListActionsCommand : Command<ListActionsCommand.Settings>
{
    public override int Execute(CommandContext context, Settings settings, CancellationToken cancellationToken)
    {
        ...
        var payload = new
        {
            ...
            example = "session create drawing.vsdx → returns {sessionId:'abc'} → shape add-shape --session abc --page-index 1 --left 1 --top 1 --width 2 --height 1 → session close --save --session abc",
            commands = all
        };
