...
description: >
  ...
  auto-layout, shapes, containers, callouts, text, ShapeSheet cells, data recordsets,
  validation, and stencil masters.
  Triggers: Visio, vsdx, diagram, page, shape, container, callout, stencil, ShapeSheet, validation.
---

# Visio MCP Server Skill

Provides 213 generated operations via Model Context Protocol. The current Visio MVP is centered on document sessions, pages, auto-layout, shapes, containers, callouts, text, ShapeSheet cells, data recordsets, validation, and stencil masters. The MCP Server forwards all requests to the shared VisioMcp Service, enabling session sharing with CLI.
...
| 9. Link data or validate | `data_recordset`, `validation` | `link-shape`, `validate` | When checking or data-linking a finished diagram |
| 10. Save & close | `file` | `close` with `save: true` | Always last |
...
Use `page`, `shape`, `container`, `text`, `cell`, and `stencil` together for the current Visio MVP:
...
| Manage containers and callouts | `container` | drop, add-member, list-members, drop-callout |
