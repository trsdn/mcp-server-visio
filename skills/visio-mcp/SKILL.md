...
Provides 101 generated operations via Model Context Protocol. The current Visio MVP is centered on document sessions, pages, shapes, text, ShapeSheet cells, and stencil masters. The MCP Server forwards all requests to the shared VisioMcp Service, enabling session sharing with CLI.
...
### Rule 4: Use Visio-Native Concepts

Work with pages, stencil masters, and ShapeSheet cells. A Visio document has no slides, layouts, or placeholders - if a workflow seems to need one, use a page, master, or cell instead.
