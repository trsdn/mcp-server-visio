...
///    when pending COM messages existed, turning the poll
...
public class VisioBatchMessagePumpTests : IAsyncLifetime
{
    ...
    public Task InitializeAsync()
    {
        ...
        _testFileCopy = Path.Join(Path.GetTempPath(), $"batch-pump-test-{Guid.NewGuid():N}.vsdx");
...
#pragma warning restore CA1031
    ...
    public void MessagePump_WhenIdle_DoesNotSpinCpu()
    {
        ...
        using (var visioProcess = Process.GetProcessById(visioPid.Value))
        {
            visioProcess.Refresh();
            visioCpuBefore = visioProcess.TotalProcessorTime;
        }
        ...
        {
            visioProcess.Refresh();
            visioCpuAfter = visioProcess.TotalProcessorTime;
        }
